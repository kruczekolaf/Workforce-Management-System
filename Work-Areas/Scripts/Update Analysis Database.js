function update_analysis_database() {
  var source_sheet = SpreadsheetApp.openById("1qvOVi5Jk86jJyvtUq4Nh0UBAo0ToaC-Zr-qk780atnw");
  var areas_sheet = source_sheet.getSheetByName("Areas");

  let last_row = areas_sheet.getLastRow();
  const working_area_range = areas_sheet.getRange(3, 1, last_row, 15);

  working_area_range.sort({column: 1, ascending: true});

  const all_employees = [];
  const employees = areas_sheet.getRange(3, 1, areas_sheet.getLastRow() - 2).getValues();
  ...
  const scheduling_sheet = SpreadsheetApp.openById("15zWq6frkb-ghUprdpDoKsAjmvpCkRv3rQKngO4jPrH4");
  ...
  categories.forEach(category => {
    ...
    const range = areas_sheet.getRange(3, col, areas_sheet.getLastRow() - 2);
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (cellValue === "able" || cellValue === "learning") {
        const row = i + 3;
        const entry = areas_sheet.getRange(row, 1).getValue(); 
